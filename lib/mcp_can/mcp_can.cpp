...
void MCP_CAN::mcp2515_read_canMsg( const INT8U buffer_sidh_addr)        /* read can msg                 */
{
    ...
    m_nDlc &= MCP_DLC_MASK;
    // MCP_DLC_MASK es 0x0F (permite 0-15), pero m_nDta solo tiene MAX_CHAR_IN_MESSAGE (8)
    // bytes. Con una trama corrupta/de bus con ruido, el registro DLC puede devolver un
    // valor de 9-15: sin este clamp, el bucle de mcp2515_readRegisterS escribe mas alla
    // de m_nDta y pisa el puntero mcpSPI (siguiente miembro en memoria), lo que provoca
    // un crash LoadProhibited en la siguiente transferencia SPI.
